...
```{r, echo = FALSE}
library(Matrix)
sceptre_output_directory <- file.path(tempdir(), "sceptre_outputs")
unlink(sceptre_output_directory, recursive = TRUE, force = TRUE)
```
...
sceptre_object <- sceptre_object |> # |> is R's base pipe, similar to %>%
  set_analysis_parameters(discovery_pairs, positive_control_pairs) |>
  run_calibration_check(print_progress = FALSE, parallel = FALSE) |>
...
write_outputs_to_directory(
  sceptre_object = sceptre_object, 
  directory = sceptre_output_directory
)
```
...
```{r}
list.files(sceptre_output_directory)
```
