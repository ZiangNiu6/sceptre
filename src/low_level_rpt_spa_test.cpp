...
// Implemented in low_level_full_test.cpp. The screened path deliberately
// reuses SCEPTRE's native statistic and fixed-count resampling implementation.
...
struct InformationStatisticCache {
  int n = 0;
  int p = 0;
...
InformationStatisticCache build_information_statistic_cache(
    const NumericVector& a,
    const NumericVector& w,
  ...
  InformationStatisticCache cache;
  cache.n = n;
  cache.p = p;
  cache.a.resize(n);
  cache.w.resize(n);
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < p; ++j) {
      ...
      cache.weighted_design(i, j) = w[i] * Z(i, j);
      if (!std::isfinite(cache.weighted_design(i, j))) {
        stop("w multiplied by Z must be finite");
      }
    }
    for (int j = 0; j < p; ++j) {
      for (int k = 0; k <= j; ++k) {
        information(j, k) += cache.weighted_design(i, j) * Z(i, k);
      }
    }
  }
  for (int j = 0; j < p; ++j) {
...
double information_statistic(
    const InformationStatisticCache& cache,
    const std::vector<int>& treated_indices,
    const int n_trt) {
  if (n_trt <= 0 || n_trt >= cache.n ||
      static_cast<int>(treated_indices.size()) < n_trt ||
      !cache.information_invertible) {
    return NA_REAL;
  }
  ...
  const double variance = weight_sum - projection;
  const double variance_scale =
      std::max(std::abs(weight_sum), std::abs(projection));
...
std::vector<double> null_information_statistics(
    const InformationStatisticCache& cache,
    const int n_trt,
    const int B,
    SEXP synthetic_idxs) {
  ...
  for (int j = 0; j < B; ++j) {
    out[j] = information_statistic(cache, (*bank)[j], n_trt);
...
void validate_treated_indices(const IntegerVector& trt_idxs,
                              const int n,
                              const int n_trt) {
  if (n_trt != trt_idxs.size() || n_trt < 0) {
    stop("n_trt must equal length(trt_idxs) and be nonnegative");
  }
  if (n_trt < 1 || n_trt >= n) {
    stop("n_trt must be between 1 and n - 1");
  }
  std::vector<int> indices(trt_idxs.size());
  for (R_xlen_t j = 0; j < trt_idxs.size(); ++j) {
    if (trt_idxs[j] == NA_INTEGER || trt_idxs[j] < 1 || trt_idxs[j] > n) {
      stop("trt_idxs must contain valid one-based indices");
    }
    indices[j] = trt_idxs[j];
  }
...
// [[Rcpp::export]]
SEXP run_low_level_test_full_rpt_spa_v1(
    NumericVector y,
    NumericVector mu,
    NumericVector a,
    NumericVector w,
  ...
  if (use_all_cells) {
    stop("RPT requires fixed-count resamples; use_all_cells must be false");
  }
  if (y.size() != a.size() || mu.size() != a.size() ||
      w.size() != a.size() || D.ncol() != a.size() ||
      Z.nrow() != a.size()) {
    stop("y, mu, a, w, D, and Z have incompatible dimensions");
  }
  validate_treated_indices(trt_idxs, a.size(), n_trt);
  ...
  std::vector<double> null_statistics = compute_null_full_statistics(
      a, w, D, 0, B1, n_trt, false, synthetic_idxs);
  ...
  std::string p_value_source =
      !observed_studentizer_valid
          ? "invalid_observed_studentizer"
          : (!B1_studentizers_valid ? "invalid_B1_studentizer"
                                    : "B1_empirical");
  ...
  double spa_count_tilt = NA_REAL;
  double spa_count_residual = NA_REAL;
  double spa_count_variance_ratio = NA_REAL;
  double spa_count_berry_esseen_ratio = NA_REAL;
  double spa_conditional_rate_correction = NA_REAL;
  ...
  if (std::isfinite(p) && p <= kTailTrigger) {
    ...
    try {
      ...
      spa_count_tilt = list_double_or_na(
          spa_diagnostics, "count_tilt");
      spa_count_residual = list_double_or_na(
          spa_diagnostics, "count_residual");
      spa_count_variance_ratio = list_double_or_na(
          spa_diagnostics, "count_variance_ratio");
      spa_count_berry_esseen_ratio = list_double_or_na(
    ...
    if (converged && std::isfinite(spa_p) && spa_p >= 0.0 && spa_p <= 1.0) {
      ...
      p = B2_studentizers_valid
              ? empirical_p_value_clamped(
                    null_statistics, z_orig, side_code)
              : NA_REAL;
      stage = 3;
      p_value_source = B2_studentizers_valid
                           ? "B2_empirical"
  ...
  List out = List::create(
      ...
      Named("spa_count_tilt") = spa_count_tilt,
      Named("spa_count_residual") = spa_count_residual,
      Named("spa_count_variance_ratio") = spa_count_variance_ratio,
      Named("spa_count_berry_esseen_ratio") =
          spa_count_berry_esseen_ratio,
      Named("spa_conditional_rate_correction") =
          spa_conditional_rate_correction,
...
// SPA-first information-studentized RPT. No permutation bank or D matrix is
// needed unless the core returns a recoverable failure.
// [[Rcpp::export]]
SEXP run_low_level_test_full_rpt_spa_always_v1(
    NumericVector y,
    NumericVector mu,
    NumericVector a,
    NumericVector w,
  ...
  if (max_iterations < 0) stop("max_iterations must be nonnegative");
  if (y.size() != a.size() || mu.size() != a.size() ||
      w.size() != a.size() || Z.nrow() != a.size()) {
    stop("y, mu, a, w, and Z have incompatible dimensions");
  }
  validate_treated_indices(trt_idxs, a.size(), n_trt);
  ...
  return List::create(
      ...
      Named("spa_root_attempted") =
          ...
          spa_reason != "invalid_fixed_margin" &&
      ...
      Named("spa_count_tilt") =
          list_double_or_na(spa_diagnostics, "count_tilt"),
      Named("spa_count_residual") =
          list_double_or_na(spa_diagnostics, "count_residual"),
      Named("spa_count_variance_ratio") =
          list_double_or_na(spa_diagnostics, "count_variance_ratio"),
      Named("spa_count_berry_esseen_ratio") =
          list_double_or_na(
              spa_diagnostics, "count_berry_esseen_ratio"),
      Named("spa_conditional_rate_correction") =
          list_double_or_na(spa_diagnostics, "conditional_rate_correction"),
      Named("spa_state") =
...
// bank row can contain a coupled maximum-size draw; only its first n_trt
// indices belong to the null law for this pair.
// [[Rcpp::export]]
SEXP finalize_low_level_test_rpt_spa_fallback_v1(
    NumericVector a,
    NumericVector w,
    NumericMatrix Z,
    int n_trt,
  ...
  if (n_trt <= 0 || n_trt >= a.size()) {
    stop("n_trt must lie strictly between zero and length(a)");
  }
  if (!spa_attempt_result.containsElementNamed("z_orig")) {
    stop("spa_attempt_result must contain z_orig");
  }

  const double z_orig = list_double_or_na(spa_attempt_result, "z_orig");
  const InformationStatisticCache cache =
      build_information_statistic_cache(a, w, Z);
  const std::vector<double> null_statistics =
      null_information_statistics(cache, n_trt, B2, synthetic_idxs);
  const bool B2_studentizers_valid = all_finite(null_statistics);
